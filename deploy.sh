...
HOST_PORT="6324"
CONTAINER_PORT="8080"
NETTY_HOST_PORT="18080"
NETTY_CONTAINER_PORT="18080"
REDIS_PORT="6379"
...
log "端口映射: ${HOST_PORT}:${CONTAINER_PORT}, ${NETTY_HOST_PORT}:${NETTY_CONTAINER_PORT}"
...
log "启动新容器，端口映射 ${HOST_PORT}:${CONTAINER_PORT} 和 ${NETTY_HOST_PORT}:${NETTY_CONTAINER_PORT}"
sudo_exec docker run -d \
  --name "${CONTAINER_NAME}" \
  --restart unless-stopped \
  -p "${HOST_PORT}:${CONTAINER_PORT}" \
  -p "${NETTY_HOST_PORT}:${NETTY_CONTAINER_PORT}" \
